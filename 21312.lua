...
local Skeleton = {
    ...
    Thickness = 1;
    DoSubsteps = true;
    TeamCheck = false; -- New flag for team check
}
...
-- New function to set team check
function Skeleton:SetTeamCheck(State)
...
function Skeleton:Update()
    ...
    -- Check TeamCheck flag
    if self.TeamCheck and self.Player.TeamColor == LocalPlayer.TeamColor then
        self:SetVisible(false);
        return;
    end
...
if false then
    ...
    for _, Player in next, game.Players:GetChildren() do
        if Player ~= LocalPlayer then
            ...
            skeleton:SetTeamCheck(true) -- Enable TeamCheck for testing
    ...
    while true do
        for _, skeleton in next, Skeletons do
            skeleton:SetColor(Color3.fromRGB(255, 0, 0)) -- Set color to red for testing
